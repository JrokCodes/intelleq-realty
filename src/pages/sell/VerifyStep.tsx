...
import { ShieldCheck, IdCard, FileText, CheckCircle2, Loader2 } from 'lucide-react';
...
export default function VerifyStep() {
  ...
  return (
    <WizardLayout
      ...
      aiHint="We use Plaid Identity for ID, then cross-check ownership against the title record. Same standards as a mortgage application."
      backTo="/sell/address"
    ...
    >
      ...
      {!verified && (
        <button
          ...
          disabled={running}
          className="w-full py-4 rounded-xl bg-sidebar text-white font-extrabold text-base hover:bg-sidebar-deep transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
        >
          {running ? (
            <>
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex items-start gap-3 animate-fade-in">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-800 leading-snug mt-0.5">
              Reviewed by Cameron Leopoldino, Lic. RB-22451.
            </p>
          </div>
        </div>
      )}
